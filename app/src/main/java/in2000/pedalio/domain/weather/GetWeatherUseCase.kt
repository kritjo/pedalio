    ...
    suspend fun getWeather(latLng: LatLng, timeDelta: Int = 0, context: Context): WeatherDataPoint {
        ...
        return when {
            ...
            timeDelta == 0 -> {
                ...
                val precipitation =
                    nowcastRepository.getPrecipitation(latLng.latitude, latLng.longitude, 0)
                val humidity =
                    nowcastRepository.getRelativeHumidity(latLng.latitude, latLng.longitude, 0)
                val windSpeed = nowcastRepository.getWindSpeed(latLng.latitude, latLng.longitude, 0)
                val windDirection =
                    nowcastRepository.getWindDirection(latLng.latitude, latLng.longitude, 0)
                val symbolCode =
                    nowcastRepository.getWeatherIcon(latLng.latitude, latLng.longitude, 0)
                WeatherDataPoint(
                    latLng,
                    temp,
                    precipitation,
            ...
            timeDelta <= 60 -> {
                ...
                val precipitation = nowcastRepository.getPrecipitation(
                    latLng.latitude,
                    latLng.longitude,
                    timeDelta
                ) // using nowcast as it is more accurate
                val humidity = locationForecastRepository.getRelativeHumidity(
                    latLng.latitude,
                    latLng.longitude,
                    0
                )
                val windSpeed =
                    locationForecastRepository.getWindSpeed(latLng.latitude, latLng.longitude, 0)
                val windDirection = locationForecastRepository.getWindDirection(
                    latLng.latitude,
                    latLng.longitude,
                    0
                )
                val symbolCode =
                    locationForecastRepository.getWeatherIcon(latLng.latitude, latLng.longitude, 0)
                WeatherDataPoint(
                    latLng,
                    temp,
                    precipitation,
            ...
            // Use LocationForecast only otherwise
                ...
                val temp =
                    locationForecastRepository.getTemp(latLng.latitude, latLng.longitude, timeDelta)
                val precipitation = locationForecastRepository.getPrecipitation(
                    latLng.latitude,
                    latLng.longitude,
                    timeDelta
                )
                ...
                val windSpeed = locationForecastRepository.getWindSpeed(
                    latLng.latitude,
                    latLng.longitude,
                    timeDelta
                )
                ...
                val symbolCode = locationForecastRepository.getWeatherIcon(
                    latLng.latitude,
                    latLng.longitude,
                    timeDelta
                )
